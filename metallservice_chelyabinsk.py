                ...
                if table:
                    ...
                    for t in tr:
                        ...
                        if 'Трубы' not in product and 'трубы' not in product:
                            continue
                        production_method = re.search(r'\w/\w|ЭСВ|ВГП', tds[0].text.replace('→', '').strip())
                        production_method = production_method[0] if production_method else None
                        production_method = "ЭСВ" if production_method == "ВГП" else production_method
                        ...
                        steel = steel if steel != '' else None
                        ...
                        if span:
                            ...
                            region = tds[4].text.strip().lower()
                        if region == 'Н.Новгород':
                            region = 'нижний новгород'
                        price_max = tds[5].text.strip().replace(u'\xa0', u'')
                        price_min = tds[7].text.strip().replace(u'\xa0', u'')
                        try:
                            price_min = float(price_min.replace('спеццена', ''))
                        except ValueError:
                            price_min = 0
                        try:
                            price_max = float(price_max.replace('спеццена', ''))
                        except ValueError:
                            price_max = 0
                        wall = float(wall.strip().replace(',', ''))
                        ...
                        insert_data('"Металлсервис" ОАО', parse_dict, html)
