        ...
        for product in product_catalog:
            ...
            for dd in data_div:
                ...
                for t in range(2, len(dd_array)):
                    wall += dd_array[t] + ' ' if dd_array[t][0].isdigit() else ''
                ...
                try:
                    diameter = float(wall[0].replace('ДУ', '').replace('ОЦ', ''))
                except:
                    diameter = None
                ...
                data_dict = {'name': type,
                             ...
                             'diameter': diameter,
                             'wall': wall_json,
                             'price_min': float_price,
                             'url': product_url,
                             'production_method': production_method}
                insert_data('"Сталепромышленная компания" АО', data_dict, data)
                print(f'diameter:{diameter}\n data{wall}')
                # main_list.append({'name': type,
                #                   'standard': None,
                #                   'steel': None,
                #                   'city': region.lower(),
                #                   'diameter': diameter,
