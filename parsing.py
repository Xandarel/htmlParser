    ...
    for d in tqdm(div):
        ...
        for sg in sub_group:
            ...
            for r in row:
                ...
                city = body[4].text.replace('Доставка в ', '').strip().lower()
                steel = body[2].text.replace('Ст', '')
                if '/' in standard:
                ...
                for s in standard:
                    for st in steel:
                        json_dict = {'name': body[0].text,
                                     ...
                                     'steel': st,
                                     'standard': s.strip().replace('с тех. протоколом', ''),
                                     'city': city,
                                     'price_min': int(body[5].text.replace(u'\xa0', '')),
                                     'url': base_url + sg,
                                     'production_method': production_method}
                        insert_data('А Групп', json_dict, browser.page_source)
